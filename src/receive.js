...
function ControlLed(e) {
	...
	if (device) {
		...
		cmd[0] = 0x02;
		cmd[1] = ledOn == 1 ? 0x01 : 0x00;
		pulseCharacteristic.writeValue(cmd)
		document.getElementById('btnLed').innerText = (e.value == 0) ? 'Led on' : 'Led off';
		e.value = 1 - ledOn;
	}
}
...
function Sampling(e) {
	...
	if (device) {
		...
		pulseCharacteristic.writeValue(cmd)
		var evt = new CustomEvent("onSamplingStarted", { detail: start });
...
function disconnect() {
	...
	document.getElementById("btnLed").disabled = true;
	document.getElementById("btnSampling").disabled = true;
	document.getElementById("btnGenerateCSV").disabled = true;
}
		...
		.then(s => {
			...
			s.getPrimaryService('f8b23a4d-89ad-4220-8c9f-d81756009f0e')
				...
				.then(pulseChs => {
					...
					document.getElementById("btnSampling").disabled = false;
					document.getElementById("btnGenerateCSV").disabled = false;
